...
public class bresult extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		try {
		 target = (int) sec.getAttribute("target");
		 wickets = (int) sec.getAttribute("wickets");
		 balls = (int) sec.getAttribute("total_balls1");
		}
		catch (Exception e) {
		...
		try {
			if (wonby.equals(team1)) {
				sql = "update  matches set team2_runs=" + target + ",team2_overs=" + balls + ",team2_wickets=" + wickets
						+ " where Team1_name='" + team1 + "' and Team2_name='" + team2
						+ "' order by match_id desc limit 1;";
			}
			else
			{
				sql = "update  matches set Team1_runs=" + target + ",team1_overs=" + balls + ",team1_wickets=" + wickets
						+ " where Team1_name='" + team1 + "' and Team2_name='" + team2
						+ "' order by match_id desc limit 1;";
			}
